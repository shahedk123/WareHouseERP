...
function newBillCaption(ref, pickerName) {
  return [
    ...
    `Reply: DONE ${ref} after Polosys update`,
  ].join('\n');
}
...
function claimReplyMsg(ref) {
  return [
    `📝 #${ref} claimed by you.`,
    `Update Polosys then reply DONE ${ref}.`,
  ].join('\n');
}
...
function nextBillCaption(bill) {
  return [
    ...
    '',
    `Reply CLAIM ${bill.ref_number} to take it.`,
    `Reply DONE ${bill.ref_number} after Polosys.`,
  ].join('\n');
}
...
function pingAccountantMsg(ref, pickerName, age, state) {
  return [
    ...
    `Please reply DONE ${ref} after updating Polosys.`,
  ].join('\n');
}
...
module.exports = {
  ...
  fmtTime, fmtDate, fmtAge, minsAgo
};
